...
class QuizViewModel: ObservableObject {
    ...
    private func setupSession() {
        let cards = wordList.pairs.map { pair in
            QuizCard(pair: pair, direction: settings.direction,
                     historicalStats: historicalWordStats[pair.id])
        }

        if settings.shuffleOrder {
            let due    = cards.filter { historicalWordStats[$0.id]?.isDue ?? true }
            let notDue = cards.filter { !(historicalWordStats[$0.id]?.isDue ?? true) }
        ...
        totalBatches = max(1, Int(ceil(Double(allCards.count) / Double(settings.batchSize))))
        loadNextBatch()
    }

    private func loadNextBatch() {
        let start = (batchIndex - 1) * settings.batchSize
        let end   = min(start + settings.batchSize, allCards.count)
        guard start < allCards.count else {
            phase = wrongCardIDs.isEmpty ? .allDone : .wrongWordsOffer
            return
        }
    ...
    private func showNextCard() {
        cardQueue.removeAll { id in
            (activeCards.first(where: { $0.id == id })?.stats.correctStreak ?? 0) >= requiredCorrect
        }

        guard !cardQueue.isEmpty else {
            let batchCorrect = activeCards.reduce(0) { $0 + $1.stats.totalCorrect }
            let batchTotal   = activeCards.reduce(0) { $0 + $1.stats.totalAttempts }
    ...
    func submitAnswer(userInput: String) {
        ...
        let normalize: (String) -> String = { $0.trimmingCharacters(in: .whitespaces).lowercased() }
        let alternatives = card.answerText.components(separatedBy: "/").map { normalize($0) }
    ...
    private func recordAnswer(card: QuizCard, isCorrect: Bool) {
        if let idx = activeCards.firstIndex(where: { $0.id == card.id }) {
            ...
            if updatedStreak < requiredCorrect {
                if !isCorrect {
                    // Re-insert 2–4 cards ahead so it's not immediately next
                    ...
                    // Correct but needs more repetitions: send to back
    ...
    private func updateProgress() {
        let done = activeCards.filter { $0.stats.correctStreak >= requiredCorrect }.count
        progress = activeCards.isEmpty ? 0 : Double(done) / Double(activeCards.count)
    }
}
