                ...
                if listStats.totalSessions > 0 {
                    VStack(alignment: .trailing, spacing: 2) {
                        Text("\(Int(listStats.accuracy * 100))%")
                            .font(.system(.title3))
                            .fontWeight(.bold)
                            .foregroundStyle(listStats.accuracy >= 0.8 ? Color.appSuccess : listStats.accuracy >= 0.5 ? .orange : Color.appError)
    ...
    private var statsSection: some View {
        if listStats.totalSessions > 0 {
            Section(lm.s.statistics) {
                StatRow(label: lm.s.totalSessions,  value: "\(listStats.totalSessions)")
                StatRow(label: lm.s.totalAnswers,   value: "\(listStats.totalAnswers)")
                StatRow(label: lm.s.correctLabel,   value: "\(listStats.totalCorrect) (\(Int(listStats.accuracy * 100))%)")
                StatRow(label: lm.s.bestStreakLabel, value: "\(listStats.bestStreak)×")
            }
        }
    }
    ...
    private var summaryRow: some View {
        HStack(spacing: 10) {
            StatPill(icon: "percent",    color: accuracyColor(listStats.accuracy),
                     label: lm.s.accuracyLabel,   value: "\(Int(listStats.accuracy * 100))%")
            StatPill(icon: "calendar",   color: Color.khaki,
                     label: lm.s.totalSessions,   value: "\(listStats.totalSessions)")
            StatPill(icon: "flame.fill", color: .orange,
                     label: lm.s.bestStreakLabel,  value: "\(listStats.bestStreak)×")
        }
        .padding(.vertical, 4)
    }
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 5) {
                    ...
                    Image(systemName: "arrow.right")
                        .font(.system(size: 9)).foregroundStyle(Color.khaki.opacity(0.5))
                    Text(pair.back)
