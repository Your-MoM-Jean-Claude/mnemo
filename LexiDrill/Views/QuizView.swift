...
struct QuizView: View {
    ...
    var body: some View {
        ...
        .animation(.spring(response: 0.40, dampingFraction: 0.80), value: showGoalComplete)
        ...
        .onChange(of: vm.phase) { newPhase in
            ...
            case .revealed(let correct):
                let style: UIImpactFeedbackGenerator.FeedbackStyle = correct ? .medium : .rigid
    ...
    private var questionView: some View {
        ...
        .onAppear { if vm.quizMode == .typing { inputFocused = true } }
    }
    ...
    private var hintRow: some View {
        if hintShown {
            ...
            Button { withAnimation(.spring(response: 0.3)) { hintShown = true } } label: {
    ...
    private var hintText: String {
        ...
        return String(first.prefix(1)) + String(repeating: "•", count: max(0, first.count - 1))
    }
            ...
            HStack(spacing: 12) {
                Button(action: { vm.flashcardAnswer(knew: false) }) {
                    Text(lm.s.flashcardDontKnow)
                        ...
                        .frame(maxWidth: .infinity)
                        .padding(.vertical, 16)
                        .background(Color.appError, in: RoundedRectangle(cornerRadius: 20))
                        .foregroundStyle(Color.appBackground)
                }
                Button(action: { vm.flashcardAnswer(knew: true) }) {
                    Text(lm.s.flashcardKnow)
                        ...
                        .frame(maxWidth: .infinity)
                        .padding(.vertical, 16)
                        .background(Color.appSuccess, in: RoundedRectangle(cornerRadius: 20))
                        .foregroundStyle(Color.appBackground)
                }
            }
            .padding(.horizontal, 20)
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 20) {
                ...
                VStack(spacing: 12) {
                    ...
                    if !correct {
                        let selected = vm.quizMode == .typing ? userInput : vm.lastSelectedOption
                        if !selected.isEmpty {
                            HStack(spacing: 4) {
    ...
    private var progressBar: some View {
        GeometryReader { geo in
            ZStack(alignment: .leading) {
                RoundedRectangle(cornerRadius: 3).fill(Color.khakiCard).frame(height: 3)
...
struct BatchCompleteView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 32) {
                ...
                ZStack {
                    Circle().stroke(Color.khakiCard, lineWidth: 14).frame(width: 140, height: 140)
                    Circle()
                        .trim(from: 0, to: accuracy)
                        .stroke(accuracyColor, style: StrokeStyle(lineWidth: 14, lineCap: .round))
                        .rotationEffect(.degrees(-90))
                        .frame(width: 140, height: 140)
                        .animation(.spring(response: 0.8), value: accuracy)
                    VStack(spacing: 2) {
                        Text("\(Int(accuracy * 100))%")
                            .font(.system(.title2, design: .rounded)).fontWeight(.bold).monospacedDigit()
                        Text("\(correct)/\(total)").font(.caption).foregroundStyle(.secondary).monospacedDigit()
                    }
                }
                if !isLast {
    ...
    var accuracyColor: Color {
        accuracy >= 0.8 ? Color.appSuccess : accuracy >= 0.5 ? .orange : Color.appError
    }
}
...
struct AllDoneView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 32) {
                VStack(spacing: 10) {
                    Text(accuracy >= 0.9 ? lm.s.excellent : accuracy >= 0.7 ? lm.s.goodJob : lm.s.tryAgain)
                        .font(.system(.largeTitle, design: .rounded)).fontWeight(.bold)
                    Text(lm.s.sessionDone).foregroundStyle(.secondary).font(.callout)
                }
                VStack(spacing: 10) {
                    ResultCard(icon: "checkmark.circle.fill", color: Color.appSuccess,
                               title: lm.s.correctLabel,   value: "\(correct) / \(total)")
                    ResultCard(icon: "percent",              color: Color.khaki,
                               title: lm.s.successRate,    value: "\(Int(accuracy * 100))%")
            ...
            VStack(spacing: 12) {
                ...
                Button(action: onSkip) {
                    Text(lm.s.skipToResultsBtn)
                        ...
                        .frame(maxWidth: .infinity)
                        .padding(.vertical, 16)
                        .background(Color.khakiCard, in: RoundedRectangle(cornerRadius: 20))
                        .foregroundStyle(.primary)
                }
            }
            .padding(.horizontal, 20)
                ...
                VStack(spacing: 28) {
                    ...
                    VStack(spacing: 10) {
                        ...
                        Text(lm.s.goalMetBody(minutesToday))
                            ...
                            .animation(.spring(response: 0.4).delay(0.28), value: appeared)
                    }
                }
