import SwiftUI
  ...
  struct QuizView: View {
      ...
      var body: some View {
          ...
          .animation(.spring(response: 0.40, dampingFraction: 0.80), value:
  showGoalComplete)
          ...
          .onChange(of: vm.phase) { newPhase in
              ...
              case .revealed(let correct):
                  let style: UIImpactFeedbackGenerator.FeedbackStyle = correct ?
  .medium : .rigid
      ...
      private var questionView: some View {
          ...
          .onAppear { if vm.quizMode == .typing && !vm.isSRS { inputFocused = true } }
      }
      ...
      private var hintRow: some View {
          if hintShown {
              ...
              Button { withAnimation(.spring(response: 0.3)) { hintShown = true } }
  label: {
      ...
      private var hintText: String {
          guard let answer = vm.currentCard?.answerText, !answer.isEmpty else { return
   "" }
          let first = answer.components(separatedBy: "/").first?
              .trimmingCharacters(in: .whitespaces) ?? answer
          return String(first.prefix(1)) + String(repeating: "•", count: max(0,
  first.count - 1))
      }
  ...
  RoundedRectangle(cornerRadius: 20))
                  ...
                  Button(action: { vm.flashcardAnswer(knew: true) }) {
                      Text(lm.s.flashcardKnow)
                          ...
                          .frame(maxWidth: .infinity)
                          .padding(.vertical, 16)
                          .background(Color.appSuccess, in:
  RoundedRectangle(cornerRadius: 20))
                          .foregroundStyle(Color.appBackground)
                  }
              }
      ...
      private var srsRatingButtons: some View {
          HStack(spacing: 10) {
              SRSRatingButton(label: lm.s.srsAgain, color: Color.appError)   {
  vm.rateSRS(.again) }
              SRSRatingButton(label: lm.s.srsHard,  color: .orange)          {
  vm.rateSRS(.hard)  }
              SRSRatingButton(label: lm.s.srsGood,  color: Color.appSuccess) {
  vm.rateSRS(.good)  }
              SRSRatingButton(label: lm.s.srsEasy,  color: .blue)            {
  vm.rateSRS(.easy)  }
          }
          .padding(.horizontal, 20)
          .padding(.bottom, 24)
      ...
      private func revealedView(correct: Bool) -> some View {
          VStack(spacing: 0) {
              ...
              VStack(spacing: 20) {
                  HStack(spacing: 8) {
                      Image(systemName: correct ? "checkmark.circle.fill" :
  "xmark.circle.fill")
                  ...
                  VStack(spacing: 12) {
                      ...
                      if !correct {
                          let selected = vm.quizMode == .typing ? userInput :
  vm.lastSelectedOption
                          if !selected.isEmpty {
                              HStack(spacing: 4) {
                                  Text(lm.s.yourAnswerWas).foregroundStyle(.secondary)

  Text(selected).strikethrough().foregroundStyle(Color.appError)
                              }
                              .font(.callout)
                          }
                      }
                  ...
                  if vm.currentStreak >= 10 {
                      HStack(spacing: 6) {
                          ...
                          Text("\(vm.currentStreak)\(lm.s.inRow)").fontWeight(.bold)
                      }
                      .font(.callout)
                      .foregroundStyle(Color.khaki)
  ...
  3).fill(Color.khakiCard).frame(height: 3)
  ...
  struct BatchCompleteView: View {
      @EnvironmentObject private var lm: LanguageManager
      let correct: Int; let total: Int; let batchIndex: Int; let totalBatches: Int
      let onContinue: () -> Void
      ...
      var body: some View {
          VStack(spacing: 0) {
              ...
              VStack(spacing: 32) {
                  ...
                  ZStack {
                      Circle().stroke(Color.khakiCard, lineWidth: 14).frame(width:
  140, height: 140)
                      Circle()
                          .trim(from: 0, to: accuracy)
                          .stroke(accuracyColor, style: StrokeStyle(lineWidth: 14,
  lineCap: .round))
  ...
  .rounded)).fontWeight(.bold).monospacedDigit()
                          Text("\(correct)/\(total)").font(.caption).foregroundStyle(.
  secondary).monospacedDigit()
                      }
                  }
                  if !isLast {
      ...
      var accuracyColor: Color {
          accuracy >= 0.8 ? Color.appSuccess : accuracy >= 0.5 ? .orange :
  Color.appError
      }
  }
  ...
  struct AllDoneView: View {
      @EnvironmentObject private var lm: LanguageManager
      let correct: Int; let total: Int; let bestStreak: Int; let onFinish: () -> Void
      ...
      var body: some View {
          VStack(spacing: 0) {
              ...
              VStack(spacing: 32) {
                  VStack(spacing: 10) {
                      Text(accuracy >= 0.9 ? lm.s.excellent : accuracy >= 0.7 ?
  lm.s.goodJob : lm.s.tryAgain)
                          .font(.system(.largeTitle, design:
  .rounded)).fontWeight(.bold)

  Text(lm.s.sessionDone).foregroundStyle(.secondary).font(.callout)
                  }
                  VStack(spacing: 10) {
                      ResultCard(icon: "checkmark.circle.fill", color:
  Color.appSuccess,
                                 title: lm.s.correctLabel,   value: "\(correct) /
  \(total)")
                      ResultCard(icon: "percent",              color: Color.khaki,
                                 title: lm.s.successRate,    value: "\(Int(accuracy *
  100))%")
                      ResultCard(icon: "flame.fill",           color: .orange,
                                 title: lm.s.bestStreakLabel, value: "\(bestStreak)×")
                  }
                  .padding(.horizontal, 20)
              }
              Spacer()
  ...
  struct ResultCard: View {
      ...
      var body: some View {
          HStack {
              Image(systemName:
  icon).font(.title2).foregroundStyle(color).frame(width: 36)
              Text(title).foregroundStyle(.secondary)
              Spacer()
              Text(value).font(.system(.title3)).fontWeight(.bold).monospacedDigit()
          }
          .padding(16)
          .glassCard(cornerRadius: 16)
      }
                  ...
                  VStack(spacing: 28) {
                      ZStack {
                          Circle()
                              ...
                              .animation(
                                  .easeInOut(duration:
  1.8).repeatForever(autoreverses: true),
                                  value: appeared
                              )
                          Circle()
  ...
  0.55), value: appeared)
                      ...
                      VStack(spacing: 10) {
                          Text(lm.s.goalMetTitle)
                              ...
                              .animation(.spring(response: 0.4).delay(0.15), value:
  appeared)
                          Text(lm.s.goalMetBody(minutesToday))
                              ...
                              .animation(.spring(response: 0.4).delay(0.28), value:
  appeared)
                      }
                  }
                  ...
                  Button(action: onDismiss) {
                      Text(lm.s.continueBtn)
                          .font(.system(.body)).fontWeight(.semibold)
                          .frame(maxWidth: .infinity).padding(.vertical, 16)
                          .background(Color.khaki, in: RoundedRectangle(cornerRadius:
  20))
                          .foregroundStyle(Color.appBackground)
                  }
                  .padding(.horizontal, 20).padding(.bottom, 24)
